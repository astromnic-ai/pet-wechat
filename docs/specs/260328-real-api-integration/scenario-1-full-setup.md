...
flowchart TD
    ...
    Guide -->|我有宠物陪伴| CollarBind[项圈绑定]
    Guide -->|开启桌面宠物| DesktopBind[桌面端绑定]
    Guide -->|跳过| Home[首页]

    CollarBind --> WiFiCollar[WiFi 配置 - 项圈]
    WiFiCollar --> WiFiResult1[配置结果]
    WiFiResult1 --> PetInfo[宠物信息录入]
    PetInfo --> PetAvatar[头像上传]
    PetAvatar --> AvatarProgress[头像进度]
    AvatarProgress -->|立即配置桌面端| DesktopBind
    AvatarProgress -->|直接进入主页| Home

    DesktopBind --> WiFiDesktop[WiFi 配置 - 桌面端]
    WiFiDesktop --> WiFiResult2[配置结果]
    WiFiResult2 --> Home
...
**所在页面：** `pages/splash/index`

**用户看到：**
- 中央显示 YEHEY 品牌 logo（爪子图标）
- 下方进度条动画加载中
- "加载中..." 提示文字

**下一步操作：**
- 无需操作。系统自动检测登录状态，约 2 秒后跳转到登录页。
...
**下一步操作：**
- 点击「微信账号登录」按钮。系统调用微信授权，登录成功后显示 "登录成功" Toast，自动跳转到引导页。

---

### 步骤 1.3 — 设备引导页（首次登录）

**所在页面：** `pages/guide/index`
...
- 上半区卡片「我有宠物陪伴」：
  ...
  - 文案 "优先配置项圈，同步宠物的真实行为"
- 下半区卡片「开启桌面宠物」：
  - 宠物剪影 + WiFi 信号图标 + 水晶球图标
  - 文案 "配置桌面端设备，开启数字宠物体验"
- 中间 "欢迎来到宠物新世界" 分隔文字
- 底部 "跳过，稍后再设置" 链接

**下一步操作：**
- 点击上半区「我有宠物陪伴」卡片，进入项圈绑定流程。
...
**下一步操作：**
- 点击按钮 → 跳转到宠物信息录入页面（携带 collarId 参数）

---

> **关于桌面端绑定的提示：** 在当前流程中，桌面端绑定安排在宠物动态定制完成之后（步骤 2.4 头像进度页提供「立即配置桌面端」入口）。用户也可以在引导页直接选择桌面端绑定，或在完成全部流程后通过首页/设备管理页随时添加。
...
- 当前关联设备信息：Collar ID: {collarId}
- "保存，下一步" 按钮（名字和品种填写后激活）
- 底部进度条（约 78% 位置）

**下一步操作：**
- 选择宠物类型 → 输入名字（如 "毛毛"）→ 输入品种（如 "英短蓝猫"）→ 可选填写其他信息
- 点击「保存，下一步」→ 系统创建宠物并关联项圈，跳转到头像上传页
...
**所在页面：** `pages/desktop-bind/index`

**用户看到：**
- 与项圈绑定页结构相同
- 标题改为 "蓝牙连接桌面端"
- 设备图标改为水晶球
- Step 1："使用磁吸充电电线给桌面端插电以后启动设备"
- Step 2：扫码或输入 MAC 地址（与项圈绑定相同交互模式）

**下一步操作：**
- 扫码/输入 MAC → 绑定成功 → 进入 WiFi 配置 → 配置成功后跳转到首页
